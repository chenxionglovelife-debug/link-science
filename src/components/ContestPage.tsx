...
export default function ContestPage({ onStartExam }: ContestPageProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'available':
        return <Badge className="bg-blue-500 hover:bg-blue-600">可开始</Badge>;
      case 'completed':
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="ml-4 flex flex-col space-y-2 items-end">
                        {paper.status === 'locked' && paper.requiredMembership && (
                          <Badge className="bg-gradient-to-r from-purple-500 to-pink-500 text-white">
                            <Crown className="w-3 h-3 mr-1" />
                            {paper.requiredMembership}
                          </Badge>
                        )}
                        <Button 
                          className={`${
                          ...
                          onClick={() => paper.status === 'locked' ? handleLockedClick() : onStartExam(paper.id)}
                        >
                          {paper.status === 'locked' && (
                            <Lock className="w-4 h-4 mr-2" />
                          )}
                          {paper.status === 'completed' ? '再次答题' : paper.status === 'locked' ? '待解锁' : '开始答题'}
                        </Button>
                      </div>
                    </div>
                  </div>
